---
title: "Session 12 tips and FAQs"
date: "2025-07-22T16:05"
categories: [FAQs]
toc-depth: 4
---
...
When you load the Natural Earth data for Exercise 12, it includes 169 columns:

![`world_shapes` and its many many columns](img/world-shapes-environment.png){width="60%"}

If you click on `world_shapes` to open it in the viewer panel and then scroll all the way to the edge to see the `geometry` column, you'll notice it's not there! It'll end at the `ISO_N3_EH` column:

![No geometry column!](img/no-geometry.png){width="90%"}

In reality, the `geometry` column *is* there, you just can't see it. RStudio only shows 50 columns at a time in the data viewer. The `geometry` column is all the way at the end (column #169), so it's not showing. You can use the little set of pagination buttons to move around to see more columns:

![Yes geometry column!](img/yes-geometry.png){width="90%"}

This gets me all the time! Once I was working on a dataset that had exactly 50 columns, and I was using `mutate()` to add a bunch of new ones, and I spent more than an hour wondering where they were all going because they weren't showing up in the data viewer! They were all on the second page of columns 🤦‍♂️.
...
world_map_with_nice_label <- world_map |> 
  mutate(nice_label = glue::glue("{NAME}<br>Population: {scales::label_comma()(POP_EST)}"))
