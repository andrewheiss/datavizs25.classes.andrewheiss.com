---
title: "Session 9 tips and FAQs"
date: "2025-07-15T09:44"
categories: [FAQs]
toc-depth: 4
---
...
In this case, you want to use `annotate()` instead. As you learned in [the lesson](/lesson/09-lesson.qmd), `annotate()` lets you add one single geom to the plot **without** needing any data mapped to it. You supply the data yourself.
...
In the FAQs for sessions 5 and 6, [I mentioned the `theme_set()` function](/news/2025-07-01_faqs_weeks-05-06.qmd#theme_set), which lets you set a theme for all plots in a document:
...
Yeah! A bunch of you ran into this! Here's a little illustration using the unemployment data from [exercise 8](/assignment/08-exercise.qmd). We'll rank all the states in the west by their unemployment rates in 2006 and 2009 and then see which ones had the biggest jumps in ranking.
...
```{r}
#| include: false
unemployment <- read_csv(here::here("projects/08-exercise/data/unemployment.csv"))
```

Before doing any pivoting, let's look at the data really quick:
...
We have a bunch of columns here, with lots of repeated data. The first two rows show Alaska, and the `state`, `region`, and `division` columns are all identical. `year` is different, since there's a row for 2006 and one for 2009, and `unemployment` and `ranking` are different, since those are the values for 2006 and 2009.
...
Now there's just one row for each state instead of two, since `state`, `region`, and `division` are identical across all the years.
...
Now we have columns for `ranking_2006` and `ranking_2009` *and* `unemployment_2006` and `unemployment_2009`, and we still have one row per state.
