...
When writing with Quarto markdown, RStudio has a neat feature that makes it easier to see what you're actualy doing. Recall from the [Markdown guide](/resource/markdown.qmd) that Markdown is a way to mark up text—it lets you make things bold or italic, add images, make headings, make lists, and so on.
...
In all your exercises, I've included three different formats in the starter .qmd file for you, like this:
...
That makes it so that HTML, PDF, and Word all appear in the dropdown menu. Additionally, it enables an automatic table of contents (`toc: true`) in each format. There are a ton of different format-specific settings you can change—[see here for more details and examples](/news/2025-06-18_cleaner-nicer-qmd-output.qmd#fancy-document-settings).


### I pasted that starter code for exercise 1 and it *still* doesn't show different formats?

In exercise 1, I gave you a big chunk of a Quarto file to paste into a document. If you pasted it *below* the existing metadata section that appears when you create a new document, like this:

![Starter code pasted to the wrong place](img/bad-pasting.png)

… then nothing will work as expected. All those title, subtitle, and format settings that *should* go up in the top area are down below the metadata area, so they're treated as part of the document itself.

The easiest solution is to make sure you're in **source** mode, select all the text there, and *then* paste it—that way it replaces the default `title: "Untitled"` stuff.
